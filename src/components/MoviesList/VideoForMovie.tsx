...
export const VideoForMovie:React.FC<IPropsVideoForMovie> = ({id}) => {
    ...
    return (
      <Button variant="contained" size="large" onClick={() => setIsOpenVideo(true)}>
          Watch the trailer <YouTubeIcon fontSize="large"/>
          <VideoModal url={`https://www.youtube.com/embed/${video[0]?.key}`} isOpenVideo={isOpenVideo} setOpenVideo={setIsOpenVideo}/>
      </Button>
    );
};
